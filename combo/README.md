...
Внутри модуля аккорды хранятся как битовые маски `ComboMask`, поэтому порядок клавиш внутри `CHORD(...)` не влияет на поиск. Синтаксис объявления аккордов при этом остается прежним.
...
**15.06.2026**:
* Поиск аккордов переведен с перебора массива клавиш на битовые маски `ComboMask`.
* Синтаксис `CHORD(...)` и `IMMEDIATE_CHORD(...)` сохранен.
